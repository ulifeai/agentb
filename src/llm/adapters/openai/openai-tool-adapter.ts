 ...
 */
export function adaptToolDefinitionToOpenAI(toolDefinition: IToolDefinition): LLMToolFunctionDefinition {
  ...
  if (!/^[a-zA-Z0-9_-]{1,64}$/.test(toolDefinition.name)) {
    console.warn(
      `[OpenAIAdapter] Tool name "${toolDefinition.name}" may not be valid for OpenAI. Sanitizing or ensuring validity beforehand is recommended. Current OpenAI constraints: 1-64 chars, a-z, A-Z, 0-9, _, -.`
    );
    // Optionally, throw error or attempt sanitization here, though `sanitizeIdForLLM` should handle it upstream.
  }

  toolDefinition.parameters.forEach((param: IToolParameter) => {
    // Validate parameter name (OpenAI specific constraints)
    if (!/^[a-zA-Z0-9_-]{1,64}$/.test(param.name)) {
      console.warn(
        `[OpenAIAdapter] Parameter name "${param.name}" for tool "${toolDefinition.name}" may not be valid for OpenAI. Ensure it meets constraints.`
      );
    }
    ...
    let paramSchemaFragment: any = param.schema || { type: param.type || 'string' }; // Default to string if type missing
    ...
    if (!paramSchemaFragment.type) {
      console.warn(
        `[OpenAIAdapter] Parameter "${param.name}" for tool "${toolDefinition.name}" has no defined type. Defaulting to 'string'.`
      );
  ...
  const openAIParametersSchema: LLMToolFunctionDefinition['parametersSchema'] = {
    type: 'object',
 ...
 */
export function adaptToolDefinitionsToOpenAI(toolDefinitions: IToolDefinition[]): LLMToolFunctionDefinition[] {
