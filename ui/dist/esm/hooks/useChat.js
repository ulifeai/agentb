...
import { streamAgentResponse, parseSSEStream,
// Import other specific event types if needed for detailed type guards
 } from '../api';
export const useChat = ({ backendUrl, initialThreadId, initialMessages = [] }) => {
    const [messages, setMessages] = useState(initialMessages);
    const [currentThreadId, setCurrentThreadId] = useState(initialThreadId || null);
    ...
    const assistantMessageAccumulatorRef = useRef('');
    const toolInteractionMessageMapRef = useRef(new Map());
    useEffect(() => {
        if (initialThreadId)
    ...
    const addChatMessage = useCallback((chatMessageData) => {
        const fullMessage = {
    ...
    const sendMessage = useCallback(async (text) => {
        ...
        try {
            ...
            for await (const event of parseSSEStream(streamReader)) {
