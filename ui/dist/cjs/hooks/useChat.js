...
const useChat = ({ backendUrl, initialThreadId, initialMessages = [] }) => {
    const [messages, setMessages] = (0, react_1.useState)(initialMessages);
    const [currentThreadId, setCurrentThreadId] = (0, react_1.useState)(initialThreadId || null);
    const [currentRunId, setCurrentRunId] = (0, react_1.useState)(null);
    ...
    const activeAssistantMessageIdRef = (0, react_1.useRef)(null);
    const assistantMessageAccumulatorRef = (0, react_1.useRef)('');
    const toolInteractionMessageMapRef = (0, react_1.useRef)(new Map());
    (0, react_1.useEffect)(() => {
        if (initialThreadId)
    ...
    const upsertMessage = (0, react_1.useCallback)((message) => {
        setMessages(prevMessages => {
            const index = prevMessages.findIndex(m => m.id === message.id);
            if (index > -1) {
                const updatedMessages = [...prevMessages];
                updatedMessages[index] = { ...updatedMessages[index], ...message }; // Ensure full ChatMessage type
    ...
    const addChatMessage = (0, react_1.useCallback)((chatMessageData) => {
        const fullMessage = {
            id: chatMessageData.id || (0, uuid_1.v4)(),
    ...
    const sendMessage = (0, react_1.useCallback)(async (text) => {
        ...
        let tempThreadId = currentThreadId || `temp-${(0, uuid_1.v4)()}`; // UI generated, will be confirmed by backend
        let activeRunId = null;
        try {
            const { streamReader } = await (0, api_1.streamAgentResponse)({
                backendUrl,
            ...
            for await (const event of (0, api_1.parseSSEStream)(streamReader)) {
                ...
                switch (event.type) {
                    ...
                    case 'thread.message.created': {
                        const createdMessage = event.data.message;
                    ...
                    case 'agent.tool.execution.started': {
                        ...
                        const updateData = {
                            ...
                            sender: 'tool_executing', status: 'in_progress',
                            timestamp: new Date(event.timestamp).toISOString(),
                            metadata: {
                        ...
                        if (msgId) {
                            upsertMessage({ id: msgId, ...updateData });
                        }
                        else {
                            addChatMessage(updateData);
                        } // Fallback
                        break;
                    }
                    case 'agent.tool.execution.completed': {
                        ...
                        const updateData = {
                            ...
                            sender: 'tool_result', status: (resultSuccess ? 'completed' : 'error'),
                            timestamp: new Date(event.timestamp).toISOString(),
                            metadata: {
                    ...
                    case 'agent.sub_agent.invocation.completed': {
                        ...
                        const updateData = {
                            ...
                            sender: 'tool_result', status: (subAgentSuccess ? 'completed' : 'error'),
                            timestamp: new Date(event.timestamp).toISOString(),
                            metadata: {
                        ...
                        if (msgId) {
                            ...
                            addChatMessage(updateData);
                        } // Fallback if initial thought wasn't mapped
