...
exports.ContextManager = exports.DEFAULT_CONTEXT_MANAGER_CONFIG = void 0;
const errors_1 = require("../core/errors");
exports.DEFAULT_CONTEXT_MANAGER_CONFIG = {
...
class ContextManager {
    constructor(messageStorage, llmClient, config = {}) {
        this.messageStorage = messageStorage;
        this.llmClient = llmClient;
        this.config = { ...exports.DEFAULT_CONTEXT_MANAGER_CONFIG, ...config };
        if (this.config.tokenThreshold <= this.config.summaryTargetTokens + this.config.reservedTokens) {
            throw new errors_1.ConfigurationError('ContextManager: tokenThreshold must be greater than summaryTargetTokens + reservedTokens.');
        }
    }
    /**
     ...
     */
    async prepareMessagesForLLM(threadId, systemPrompt, // Expects a fully formed LLMMessage for the system prompt
    currentTurnMessages = [] // User input, previous assistant partial responses etc. for current turn
    ) {
        ...
        let messagesToConsider = [];
        const summaryIndex = historyLLMMessages.findIndex((m) => m.role === 'system' &&
            m.content &&
            typeof m.content === 'string' &&
            m.content.includes('======== CONVERSATION HISTORY SUMMARY ========')); // A bit fragile
        if (summaryIndex !== -1) {
        ...
        let currentMessagesForLLM = [systemPrompt, ...messagesToConsider, ...currentTurnMessages];
        ...
        if (currentTokenCount > this.config.tokenThreshold) {
            console.info(`[ContextManager] Thread ${threadId} token count ${currentTokenCount} exceeds threshold ${this.config.tokenThreshold}. Attempting summarization.`);
            ...
            if (messagesToSummarize.length > 1) {
                ...
                if (summaryText) {
                    ...
                    console.warn(`[ContextManager] Summarization failed for thread ${threadId}. Proceeding with potentially truncated context.`);
                }
            }
            else {
        ...
        while (currentTokenCount > this.config.tokenThreshold - this.config.reservedTokens &&
            ...
            if (!removed && currentMessagesForLLM.length > 1 + currentTurnMessages.length) {
                ...
                console.warn('[ContextManager] Context still too long after attempting to remove oldest messages. System prompt or current input might be too large.');
                break;
            }
            if (!removed && currentMessagesForLLM.length <= 1 + currentTurnMessages.length)
                break; // Nothing left to remove except system/current turn
            currentTokenCount = await this.llmClient.countTokens(currentMessagesForLLM, this.config.summarizationModel);
    ...
    async generateSummary(messages) {
        ...
        try {
            const response = (await this.llmClient.generateResponse(summarizationPrompt, {
                ...
                stream: false,
            })); // Assert non-streaming response type
    ...
    mapIMessageToLLMMessage(message) {
        ...
        const llmMessage = {
            role: message.role,
            content: message.content, // Cast needed if IMessage.content is broader
        };
        if (message.metadata?.tool_calls) {
            llmMessage.tool_calls = message.metadata.tool_calls;
        }
        if (message.metadata?.tool_call_id) {
            llmMessage.tool_call_id = message.metadata.tool_call_id;
        }
        if (message.role === 'tool' && message.metadata?.name) {
            // OpenAI expects 'name' for tool role in some contexts
            llmMessage.name = message.metadata.name;
        }
        return llmMessage;
    }
}
