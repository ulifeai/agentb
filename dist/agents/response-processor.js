...
exports.LLMResponseProcessor = void 0;
...
class LLMResponseProcessor {
    ...
    // toolProvider?: IToolProvider // Optional: if validation of tool names is done during parsing
        ...
        this.xmlBuffer = '';
        this.openXmlTags = []; // Stack for nested XML tags
        this.config = {
     ...
     */
    processCompleteResponse(llmMessageContent, llmToolCalls) {
        const events = [];
    ...
    async *processStream(llmStream) {
        ...
        try {
            for await (const chunk of llmStream) {
                ...
                if (this.config.enableNativeToolCalling && chunk.tool_calls) {
                    for (const tcChunk of chunk.tool_calls) {
                        ...
                        if (tcChunk.id)
                            buffer.id = tcChunk.id;
                        if (tcChunk.type)
                            buffer.type = tcChunk.type;
                        if (tcChunk.function?.name)
                            buffer.function.name = (buffer.function.name || '') + tcChunk.function.name;
                        if (tcChunk.function?.arguments)
                            buffer.function.arguments = (buffer.function.arguments || '') + tcChunk.function.arguments;
                    }
                }
                    ...
                    for (const index in toolCallBuffers) {
                        ...
                        if (bufferedCall.id && bufferedCall.function?.name && bufferedCall.function?.arguments) {
                            try {
                                ...
                                const parseError = new errors_1.LLMError(`Failed to parse JSON arguments for tool call ${bufferedCall.function.name || 'unknown'}: ${e.message}`);
                                yield { type: 'error', error: parseError };
                                console.error(`[LLMResponseProcessor] Error parsing tool arguments:`, bufferedCall.function.arguments, parseError);
                            }
                        }
                        else {
                            // Incomplete tool call when stream ended
                            const incompleteError = new errors_1.LLMError(`Incomplete tool call data at end of stream for index ${index}.`);
            ...
            const processingError = new errors_1.LLMError(`Error processing LLM stream: ${error.message}`, undefined, {
                originalError: error,
            });
...
exports.LLMResponseProcessor = LLMResponseProcessor;
