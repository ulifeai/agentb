...
exports.DelegateToSpecialistTool = void 0;
...
class DelegateToSpecialistTool {
    constructor(dependencies) {
        this.toolName = 'delegateToSpecialistAgent'; // Fixed name for this meta-tool
        this.dependencies = dependencies;
        if (!dependencies.getDefaultRunConfig) {
            throw new errors_1.ConfigurationError('DelegateToSpecialistTool requires getDefaultRunConfig in dependencies.');
        }
    }
    async getDefinition() {
    ...
    async execute(input, plannerAgentContext // Optional: context of the calling planner agent for logging/tracing
        ...
        const specialistSystemPrompt = (0, prompt_builder_1.generateToolsSystemPrompt)(toolset.name, toolset.description, await Promise.all(toolset.tools.map((t) => t.getDefinition())), toolset.metadata?.apiTitle
        ...
        const specialistResponseProcessor = new response_processor_1.LLMResponseProcessor(specialistAgentRunConfig.responseProcessorConfig);
        const specialistToolExecutor = new tool_executor_1.ToolExecutor(specialistToolProvider, specialistAgentRunConfig.toolExecutorConfig);
        ...
        const specialistStorage = new memory_storage_1.MemoryStorage();
        ...
        const specialistContextManager = new context_manager_1.ContextManager(specialistMessageStorage, llmClient, specialistAgentRunConfig.contextManagerConfig);
        // 4. Create IAgentContext for the specialist sub-agent
        const subRunId = `subrun-${plannerRunId}-${specialistId.substring(0, 8)}-${(0, uuid_1.v4)().substring(0, 4)}`;
        ...
        const WorkerAgentClass = workerAgentImplementation || agents_1.BaseAgent;
...
exports.DelegateToSpecialistTool = DelegateToSpecialistTool;
