...
exports.OpenAPIConnector = exports.OpenAPIOperationTool = void 0;
const spec_parser_1 = require("./spec-parser");
const utils_1 = require("./utils");
const utils_2 = require("../core/utils");
const generic_http_tool_1 = require("./tools/generic-http-tool");
/**
...
class OpenAPIOperationTool {
    constructor(operation, specParser, connector) {
        if (!operation.operationId) {
            // This should ideally be caught earlier, but defensive check here.
            throw new Error(`Operation for ${operation.method} ${operation.path} is missing an operationId and cannot be created as a tool.`);
        }
    ...
    createDefinition() {
        ...
        try {
            ...
            console.warn(`[OpenAPIOperationTool] Could not generate parameters schema for ${this.operation.operationId}: ${e.message}. Tool will have minimal parameters.`);
        ...
        return {
            name: (0, utils_2.sanitizeIdForLLM)(this.operation.operationId),
...
exports.OpenAPIOperationTool = OpenAPIOperationTool;
/**
...
class OpenAPIConnector {
    ...
    async _initialize(options) {
        try {
            if (options.spec) {
                ...
                this.spec = await (0, utils_1.fetchSpec)(options.specUrl);
            }
            else {
                throw new Error('Spec or specUrl is required for initialization.');
            }
            this.specParser = new spec_parser_1.OpenAPISpecParser(this.spec, this.tagFilter);
            this._isInitialized = true;
            console.info(`[OpenAPIConnector] Initialized successfully${this.tagFilter ? ` for tag "${this.tagFilter}"` : ' (full spec)'}. Auth type: ${this.authentication.type}`);
            this.generateTools();
        }
        catch (error) {
            this._isInitialized = false;
    ...
    generateTools() {
        ...
        if (this.includeGenericTool && !this.tagFilter) {
            ...
            this.providedTools.push(new generic_http_tool_1.GenericHttpApiTool(this));
        }
    }
    ...
    setAuthentication(auth) {
        ...
        console.info(`[OpenAPIConnector${this.tagFilter ? ` (${this.tagFilter})` : ''}${initStatus}] Authentication updated to type: ${this.authentication.type}`);
    }
    async getTools() {
    ...
    /** @internal */
    async executeSpecificOperationInternal(sanitizedToolName, args, agentContext) {
        this.assertInitialized();
        ...
        const operation = operationToolInstance.operation; // Needs change in OpenAPIOperationTool
        if (!operation) {
        ...
        const queryForUrl = {};
        const headersForRequest = { 'accept': 'application/json' };
        let bodyForRequest = undefined;
    ...
    /** @internal */
    async executeGenericOperationInternal(args, agentContext) {
        this.assertInitialized();
        ...
        const requestOptions = { method: method.toUpperCase(), headers: new Headers(headers) };
        ...
        console.debug(`[OpenAPIConnector] Executing Generic Request: ${opLabel}, URL: ${finalUrl}`, requestBody ? `with body (type ${typeof requestBody})` : 'No Body', `and headers: ${JSON.stringify((0, utils_1.headersToObject)(requestOptions.headers))}`);
        return this.makeApiCall(finalUrl, requestOptions, opLabel);
    }
    async applyAuthentication(headers, currentUrl, agentContext) {
        let urlToUse = currentUrl;
        let authToApply = this.authentication; // Start with static
        ...
        switch (authToApply.type) {
            ...
            case 'none':
                break;
            default:
                console.warn(`[OpenAPIConnector:${this.sourceId}] Unknown auth type: ${authToApply?.type}`);
        }
        return { url: urlToUse };
    }
    async makeApiCall(url, options, operationLabel) {
        let response;
        try {
            ...
            console.error(`[OpenAPIConnector] Network error during API call (${operationLabel}) to ${url}: ${message}`, options);
            return {
                success: false,
                data: null,
        ...
        try {
            ...
            console.warn(`[OpenAPIConnector] API call (${operationLabel}) to ${url} successful (status ${response.status}) but failed to parse response as ${contentType}. Error: ${e.message}. Attempting to return raw text.`);
            try {
                ...
                console.error(`[OpenAPIConnector] Failed to even get raw text for response of (${operationLabel}): ${textError.message}`);
                return {
                    success: false,
                    data: null,
...
exports.OpenAPIConnector = OpenAPIConnector;
