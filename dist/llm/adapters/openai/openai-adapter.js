 ...
 */
const openai_1 = __importDefault(require("openai")); // Official OpenAI SDK
const openai_tool_adapter_1 = require("./openai-tool-adapter");
const errors_1 = require("../../../core/errors"); // Correct path to core/errors
class OpenAIAdapter {
    constructor(options) {
        const apiKey = options?.apiKey || process.env.OPENAI_API_KEY;
        if (!apiKey) {
            throw new errors_1.ConfigurationError('OpenAI API key is required. Provide it in options or set OPENAI_API_KEY environment variable.');
        }
        ...
        console.info(`[OpenAIAdapter] Initialized with default model: ${this.defaultModel}. BaseURL: ${options?.baseURL || 'OpenAI Default'}`);
    }
    /**
    ...
    async generateResponse(messages, options) {
        const { model: _m, tools, tool_choice, stream, temperature, max_tokens, systemPrompt, ...otherOptions } = options;
        const modelToUse = options.model || this.defaultModel;
        let preparedMessages = messages.map((msg) => this.mapToOpenAIMessageParam(msg) // Use the corrected mapping function
        );
        if (options.systemPrompt) {
            ...
            preparedMessages.unshift(this.mapToOpenAIMessageParam({ role: 'system', content: 'You are a helpful assistant.' }));
        }
        ...
        if (options.tools) {
            console.info(`[OpenAIAdapter] Tools configured:`, options.tools.length);
        }
        if (options.stream) {
            requestPayload.stream = true;
            return this.streamOpenAIResponse(requestPayload);
        }
        else {
            try {
                const completion = await this.openai.chat.completions.create(requestPayload);
                if (!completion.choices || completion.choices.length === 0 || !completion.choices[0].message) {
                    throw new errors_1.LLMError('OpenAI response missing choices or message.', 'api_error', { response: completion });
                }
                ...
                throw new errors_1.LLMError('Unhandled error after OpenAI non-streaming call: ' + (error.message || 'Unknown error'));
            }
        }
    }
    /**
     ...
     */
    mapToOpenAIMessageParam(message) {
        switch (message.role) {
            case 'system':
                if (typeof message.content !== 'string')
                    throw new errors_1.ConfigurationError('System message content must be a string.');
            ...
            case 'user':
                ...
                if (typeof message.content === 'string') {
                    ...
                    const mappedContent = message.content.map((part) => {
                        if (part.type === 'text')
                            return { type: 'text', text: part.text };
                        if (part.type === 'image_url')
                            return { type: 'image_url', image_url: part.image_url }; // Assuming structure matches
                        throw new errors_1.ConfigurationError(`Unsupported user message content part type: ${part.type}`);
                    });
                    return { role: 'user', content: mappedContent, name: message.name };
                }
                throw new errors_1.ConfigurationError('User message content must be a string or valid array of parts.');
            case 'assistant':
            ...
            case 'tool':
                if (!message.tool_call_id)
                    throw new errors_1.ConfigurationError("Message with role 'tool' must have a 'tool_call_id'.");
                if (typeof message.content !== 'string')
                    throw new errors_1.ConfigurationError('Tool message content must be a string for OpenAI.');
                return {
                    role: 'tool',
            ...
            default:
                ...
                throw new errors_1.ConfigurationError(`Unsupported message role: ${message.role}`);
        }
    }
    /**
     * Maps OpenAI's ChatCompletionMessage back to our generic LLMMessage.
     */
    mapFromOpenAIChatCompletionMessage(openAIMessage, usage // TODO: Propagate usage if needed
    ) {
        const message = {
            role: openAIMessage.role, // 'assistant' typically from response
     ...
     */
    mapToOpenAIToolChoice(toolChoice) {
        if (typeof toolChoice === 'string') {
            ...
            console.warn(`[OpenAIAdapter] Invalid string for LLMToolChoice "${toolChoice}". Defaulting to 'auto'. Valid strings: 'auto', 'none', 'required'.`);
            return 'auto';
        }
        ...
        throw new errors_1.ConfigurationError(`Invalid object structure for LLMToolChoice: ${JSON.stringify(toolChoice)}`);
    }
    /**
     * Handles streaming responses from OpenAI.
     */
    async *streamOpenAIResponse(requestPayload) {
        try {
            ...
            for await (const chunk of stream) {
                ...
                if (!chunk.choices || chunk.choices.length === 0 || !chunk.choices[0].delta) {
                    if (chunk.usage)
                        yield { usage: chunk.usage }; // Handle usage-only chunks
                    continue;
                }
                ...
                if (delta.tool_calls) {
                    mappedChunk.tool_calls = delta.tool_calls
                        ...
                        .filter((tc) => tc !== null && (tc.id || tc.function?.name || tc.function?.arguments)); // Ensure type and filter out empty
                    if (mappedChunk.tool_calls && mappedChunk.tool_calls.length === 0) {
                        delete mappedChunk.tool_calls;
                    }
                }
                if (finish_reason)
                    mappedChunk.finish_reason = finish_reason;
                if (chunk.usage)
                    mappedChunk.usage = chunk.usage;
    ...
    async countTokens(messages, model) {
        ...
        console.warn(`[OpenAIAdapter] Token counting for model "${model}" is using a rough estimate. ` +
     ...
     */
    formatToolsForProvider(toolDefinitions) {
        const adaptedProviderFormats = (0, openai_tool_adapter_1.adaptToolDefinitionsToOpenAI)(toolDefinitions);
    ...
    handleOpenAIError(error) {
        ...
        if (error instanceof openai_1.default.APIError) {
            ...
            const errorType = error.type || error.code || 'openai_api_error';
            ...
            throw new errors_1.LLMError(message, errorType, {
                statusCode,
                provider: 'openai',
                headers: error.headers,
            ...
            throw new errors_1.LLMError(error.message || 'An unexpected error occurred while interacting with OpenAI.', 'sdk_error', {
                provider: 'openai',
            ...
            throw new errors_1.LLMError('An unknown error occurred with OpenAI.', 'unknown_sdk_error', {
                provider: 'openai',
