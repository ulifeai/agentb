...
exports.ROUTER_TOOL_NAME = exports.GENERIC_HTTP_TOOL_NAME = void 0;
exports.generateToolsSystemPrompt = generateToolsSystemPrompt;
exports.generateGenericHttpToolSystemPrompt = generateGenericHttpToolSystemPrompt;
exports.generateRouterSystemPrompt = generateRouterSystemPrompt;
...
exports.GENERIC_HTTP_TOOL_NAME = 'genericHttpRequest';
exports.ROUTER_TOOL_NAME = 'routeToToolset'; // Changed from 'delegateToSpecialist' to avoid confusion
/**
 ...
 */
function formatIToolParameterForPrompt(param) {
 ...
 */
function generateToolsSystemPrompt(toolSetName, toolSetDescription, toolDefinitions, apiInfo, // Generic API info
baseUrl, // Could be part of a more generic API descriptor
businessContextText) {
 ...
 */
function formatOpenAPIParameterForGenericPrompt(param) {
 ...
 */
function generateGenericHttpToolSystemPrompt(operations, // From OpenAPISpecParser
apiInfo, baseUrl, businessContextText) {
...
To interact with this API, you MUST use the "${exports.GENERIC_HTTP_TOOL_NAME}" tool.
...
Below is a list of available API operations. Review these carefully to determine the correct method, path, and parameters for the "${exports.GENERIC_HTTP_TOOL_NAME}" tool.
...
`;
    if (operations.length === 0) {
        ...
        operations.forEach((op) => {
            ...
            if (op.description && op.description !== op.summary)
                prompt += `  Description: ${op.description}\n`;
            prompt += `  HTTP Method: ${op.method}\n`;
            prompt += `  Relative Path: ${op.path}\n`;
            const pathParams = op.parameters.filter((p) => p.in === 'path');
            if (pathParams.length > 0) {
                prompt += `  Path Parameters (to be substituted into the 'path' string for ${exports.GENERIC_HTTP_TOOL_NAME}):\n`;
                pathParams.forEach((p) => (prompt += `    ${formatOpenAPIParameterForGenericPrompt(p)}\n`));
            }
            const queryParams = op.parameters.filter((p) => p.in === 'query');
            if (queryParams.length > 0) {
                prompt += `  Query Parameters (provide in the 'queryParams' object for ${exports.GENERIC_HTTP_TOOL_NAME}):\n`;
            ...
            if (headerParams.length > 0) {
                prompt += `  Header Parameters (can be provided in 'headers' object for ${exports.GENERIC_HTTP_TOOL_NAME} if not automatically handled):\n`;
                headerParams.forEach((p) => (prompt += `    ${formatOpenAPIParameterForGenericPrompt(p)}\n`));
            }
            if (op.requestBodySchema) {
                prompt += `  Request Body (provide in the 'requestBody' field for ${exports.GENERIC_HTTP_TOOL_NAME}, typically as a JSON object):\n`;
    ...
    prompt += `\nAlways use the "${exports.GENERIC_HTTP_TOOL_NAME}" tool to make API calls. Ensure all parameters are correctly formatted.`;
    return prompt;
}
/**
 ...
 */
async function generateRouterSystemPrompt(availableToolSets, apiInfo, // Generic API info
businessContextText) {
...
Your primary task is to understand user requests and delegate them to the most appropriate specialist Toolset using the "${exports.ROUTER_TOOL_NAME}" tool.
  
The "${exports.ROUTER_TOOL_NAME}" tool requires these arguments:
...
`;
    ...
    prompt += `
To delegate effectively using the "${exports.ROUTER_TOOL_NAME}" tool, follow these steps:
...
5.  Invoke the "${exports.ROUTER_TOOL_NAME}" tool with 'toolSetId', 'toolName', and 'toolParameters'.
