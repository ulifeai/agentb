...
export class OpenAIAdapter {
    constructor(options) {
        ...
        if (!apiKey) {
            throw new ConfigurationError('OpenAI API key is required. Provide it in options or set OPENAI_API_KEY environment variable.');
        }
        ...
        switch (message.role) {
            ...
            case 'user':
                ...
                if (typeof message.content === 'string') {
                    ...
                    const mappedContent = message.content.map((part) => {
                        ...
                        throw new ConfigurationError(`Unsupported user message content part type: ${part.type}`);
                    });
            ...
            case 'tool':
                if (!message.tool_call_id)
                    throw new ConfigurationError("Message with role 'tool' must have a 'tool_call_id'.");
                if (typeof message.content !== 'string')
            ...
            default:
                ...
                throw new ConfigurationError(`Unsupported message role: ${message.role}`);
        }
    }
    /**
