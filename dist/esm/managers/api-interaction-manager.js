...
import { generateGenericHttpToolSystemPrompt, generateRouterSystemPrompt, ROUTER_TOOL_NAME as LLM_ROUTER_TOOL_NAME, // Constant for the router tool's name
 } from '../llm/prompt-builder';
import { ToolsetOrchestrator } from './toolset-orchestrator';
import { OpenAPIConnector } from '../openapi/connector';
import { DEFAULT_AGENT_RUN_CONFIG, BaseAgent, // Using BaseAgent as the default concrete implementation
...
import { DelegateToSpecialistTool } from '../tools/core/delegate-to-specialist-tool'; // New import
...
export class ApiInteractionManager {
    constructor(options) {
        ...
        if (!this.defaultAgentRunConfig.model) {
            ...
            throw new ConfigurationError('A default agent model must be specified either in library defaults or ApiInteractionManagerOptions.defaultAgentRunConfig.');
        }
        ...
        if (this.mode === 'hierarchicalPlanner' && !options.toolsetOrchestratorConfig) {
            throw new ConfigurationError("ToolsetOrchestratorConfig is required for 'hierarchicalPlanner' mode (to provide specialists).");
    ...
    async _initialize() {
        ...
        if (this.mode === 'genericOpenApi') {
            ...
            if (!this.options.genericOpenApiProviderConfig) {
                ...
                throw new InvalidStateError("GenericOpenApiProviderConfig is missing for 'genericOpenApi' mode during initialization.");
            }
    ...
    async getPrimaryLLMFormattedTools() {
        ...
        if (this.mode === 'genericOpenApi') {
            if (!this.genericToolProvider)
                throw new InvalidStateError('Generic tool provider not available.');
            ...
            if (!this.toolsetOrchestrator)
                throw new InvalidStateError('Toolset orchestrator not available.');
    ...
    async getPrimaryLLMSystemPrompt(customBusinessContext) {
        ...
        if (this.mode === 'genericOpenApi') {
            if (!this.genericToolProvider || !(this.genericToolProvider instanceof OpenAPIConnector)) {
                throw new InvalidStateError("Generic tool provider (must be OpenAPIConnector) not available or of wrong type in 'genericOpenApi' mode for prompt generation.");
            }
    ...
    createDelegateToSpecialistTool() {
        if (!this.toolsetOrchestrator) {
            throw new InvalidStateError("ToolsetOrchestrator is required to create DelegateToSpecialistTool but it's not initialized.");
        }
        const deps = {
    ...
    createRouterToolProvider(orchestrator) {
        ...
        return {
            ...
            getTool: async (name) => (name === LLM_ROUTER_TOOL_NAME ? routerTool : undefined),
        };
    }
    /**
    ...
    async *runAgentInteraction(threadId, initialTurnMessages, agentRunConfigOverride = {}, existingRunId) {
        ...
        if (this.mode === 'genericOpenApi') {
            ...
            const effectivelyPlanningAgent = (this.agentImplementation.name === BaseAgent.name && !this.options.agentImplementation) || // It's the default BaseAgent
        ...
        if (existingRunId) {
            ...
            if (!continuableStates.includes(existingRecord.status)) {
                throw new InvalidStateError(`Cannot continue run: Agent run "${existingRunId}" is not in a continuable state (current status: ${existingRecord.status}).`);
            }
        ...
        const contextManager = new ContextManager(this.messageStorage, this.llmClient, effectiveRunConfig.contextManagerConfig // Pass ContextManager specific config
        );
        const agentContext = {
    ...
    toolOutputs, agentRunConfigOverride = {}) {
        ...
        if (existingRun.status !== 'requires_action') {
            throw new InvalidStateError(`Cannot continue: Agent run "${runId}" is not in 'requires_action' state (current status: ${existingRun.status}).`);
        }
        if (existingRun.threadId !== threadId) {
            throw new ConfigurationError(`Thread ID mismatch for run "${runId}". Expected "${existingRun.threadId}", got "${threadId}".`);
        }
        ...
        if (this.mode === 'genericOpenApi') {
            ...
            const effectivelyPlanningAgent = (this.agentImplementation.name === BaseAgent.name && !this.options.agentImplementation) ||
        ...
        const responseProcessor = new LLMResponseProcessor(effectiveRunConfig.responseProcessorConfig);
        const toolExecutor = new ToolExecutor(agentToolProvider, effectiveRunConfig.toolExecutorConfig);
        const contextManager = new ContextManager(this.messageStorage, this.llmClient, effectiveRunConfig.contextManagerConfig);
        const agentContext = {
            runId,
            threadId,
            llmClient: this.llmClient,
        ...
        if (!agent.submitToolOutputs) {
            throw new InvalidStateError(`The configured agent implementation ("${AgentToRunClass.name}") does not support 'submitToolOutputs'.`);
        }
    ...
    async updateAuthentication(newAuth) {
        ...
        if (this.mode === 'genericOpenApi') {
            if (typeof newAuth === 'function') {
                ...
                const currentOpts = this.genericToolProvider instanceof OpenAPIConnector
            ...
            if (typeof newAuth !== 'function') {
                throw new ConfigurationError("For 'toolsetsRouter' mode, updateAuthentication expects a callback function: " +
                    '(sourceId: string, currentOptions: OpenAPIConnectorOptions) => ConnectorAuthentication | undefined.');
            }
            if (!this.toolsetOrchestrator) {
