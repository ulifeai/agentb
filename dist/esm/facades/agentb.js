...
import { OpenAIAdapter } from '../llm/adapters/openai/openai-adapter';
import { MemoryStorage } from '../threads/storage/memory-storage';
import { DEFAULT_AGENT_RUN_CONFIG } from '../agents';
import { ConfigurationError, InvalidStateError } from '../core/errors';
...
export class AgentBInternal {
    ...
    initialize(options = {}) {
        ...
        if (llmConfig.provider.toLowerCase() === 'openai') {
            ...
            if (!openaiApiKey) {
                throw new ConfigurationError("OpenAI API key is required. Set OPENAI_API_KEY or provide in llmProvider.apiKey.");
            }
        ...
        if (!this.globalDefaultAgentRunConfig.model) {
            throw new ConfigurationError('A default agent model must be specified.');
        }
