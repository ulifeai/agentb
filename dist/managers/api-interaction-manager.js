...
exports.ApiInteractionManager = void 0;
const openai_tool_adapter_1 = require("../llm/adapters/openai/openai-tool-adapter");
const prompt_builder_1 = require("../llm/prompt-builder");
const toolset_orchestrator_1 = require("./toolset-orchestrator");
const connector_1 = require("../openapi/connector");
const agents_1 = require("../agents"); // Correct path to agents module index
const response_processor_1 = require("../agents/response-processor");
const tool_executor_1 = require("../agents/tool-executor");
const context_manager_1 = require("../agents/context-manager");
const memory_storage_1 = require("../threads/storage/memory-storage"); // Default in-memory storage
const aggregated_tool_provider_1 = require("../tools/core/aggregated-tool-provider");
const errors_1 = require("../core/errors");
const uuid_1 = require("uuid"); // For generating unique run IDs
const delegate_to_specialist_tool_1 = require("../tools/core/delegate-to-specialist-tool"); // New import
const planning_agent_1 = require("../agents/planning-agent"); // New import
/**
...
class ApiInteractionManager {
    constructor(options) {
        ...
        this.messageStorage = options.messageStorage || new memory_storage_1.MemoryStorage();
        this.agentRunStorage = options.agentRunStorage || new memory_storage_1.MemoryStorage();
        this.threadStorage = options.threadStorage || new memory_storage_1.MemoryStorage();
        // Resolve the default agent run configuration
        this.defaultAgentRunConfig = {
            ...agents_1.DEFAULT_AGENT_RUN_CONFIG, // Start with library defaults
            model: options.defaultAgentRunConfig?.model || agents_1.DEFAULT_AGENT_RUN_CONFIG.model || 'gpt-4o-mini', // Ensure a model
            ...(options.defaultAgentRunConfig || {}), // Apply user-defined defaults
        };
        if (!this.defaultAgentRunConfig.model) {
            // Final check after merging
            throw new errors_1.ConfigurationError('A default agent model must be specified either in library defaults or ApiInteractionManagerOptions.defaultAgentRunConfig.');
        }
        this.agentImplementation = options.agentImplementation || agents_1.BaseAgent;
        // Validate mode-specific configurations
        if (this.mode === 'toolsetsRouter' && !options.toolsetOrchestratorConfig) {
            throw new errors_1.ConfigurationError("ToolsetOrchestratorConfig is required for 'toolsetsRouter' mode.");
        }
        if (this.mode === 'hierarchicalPlanner' && !options.toolsetOrchestratorConfig) {
            throw new errors_1.ConfigurationError("ToolsetOrchestratorConfig is required for 'hierarchicalPlanner' mode (to provide specialists).");
        } // Planner also needs toolsets
        if (this.mode === 'genericOpenApi' && !options.genericOpenApiProviderConfig) {
            throw new errors_1.ConfigurationError("GenericOpenApiProviderConfig is required for 'genericOpenApi' mode.");
        }
    }
    /**
    ...
    async _initialize() {
        ...
        if (this.mode === 'toolsetsRouter' || this.mode === 'hierarchicalPlanner') {
            if (!this.options.toolsetOrchestratorConfig) {
                ...
                throw new errors_1.InvalidStateError('ToolsetOrchestratorConfig is missing for initialization.');
            }
            // Pass this.llmClient to the ToolsetOrchestrator constructor
            this.toolsetOrchestrator = new toolset_orchestrator_1.ToolsetOrchestrator(this.options.toolsetOrchestratorConfig, this.llmClient);
            ...
            if (individualProviders && individualProviders.length > 0) {
                this.aggregatedMasterToolProvider = new aggregated_tool_provider_1.AggregatedToolProvider(individualProviders);
                await this.aggregatedMasterToolProvider.ensureInitialized();
                ...
                this.aggregatedMasterToolProvider = new aggregated_tool_provider_1.AggregatedToolProvider([]);
                await this.aggregatedMasterToolProvider.ensureInitialized();
        ...
        if (this.mode === 'genericOpenApi') {
            let connectorOptsToUse = this.options.genericOpenApiProviderConfig;
            ...
            if (!connectorOptsToUse && this.options.toolsetOrchestratorConfig && this.options.toolsetOrchestratorConfig.length > 0) {
                ...
                if (firstSourceConfig.type === 'openapi' || firstSourceConfig.type === undefined) {
                    connectorOptsToUse = {
                        ...firstSourceConfig.openapiConnectorOptions,
            ...
            if (!connectorOptsToUse) {
                throw new errors_1.ConfigurationError('ApiInteractionManager: genericOpenApi mode requires either genericOpenApiProviderConfig or a valid first provider in toolsetOrchestratorConfig.');
            }
            if (!connectorOptsToUse.sourceId) {
                throw new errors_1.ConfigurationError('ApiInteractionManager: genericOpenApi mode requires sourceId in connector options.');
            }
            ...
            if (!this.options.genericOpenApiProviderConfig) {
                ...
                throw new errors_1.InvalidStateError("GenericOpenApiProviderConfig is missing for 'genericOpenApi' mode during initialization.");
            }
            ...
            this.genericToolProvider = new connector_1.OpenAPIConnector(connectorOpts);
    ...
    async ensureInitialized() {
        ...
        if (!this._isInitialized) {
            ...
            throw new errors_1.ApplicationError('ApiInteractionManager could not be initialized. Check previous logs.');
        }
    }
    /**
    ...
    assertInitialized() {
        if (!this._isInitialized) {
            throw new errors_1.InvalidStateError('ApiInteractionManager is not initialized. Call ensureInitialized() first.');
        }
    }
    /**
     ...
     */
    async getPrimaryLLMFormattedTools() {
        this.assertInitialized();
        let definitionsToFormat;
        if (this.mode === 'genericOpenApi') {
            if (!this.genericToolProvider)
                throw new errors_1.InvalidStateError('Generic tool provider not available.');
            ...
            if (!this.toolsetOrchestrator)
                throw new errors_1.InvalidStateError('Toolset orchestrator not available.');
            ...
            if (!this.toolsetOrchestrator)
                throw new errors_1.InvalidStateError('Toolset orchestrator not available for hierarchical planner mode.');
            ...
            throw new errors_1.InvalidStateError(`Tool formatting not implemented for mode: ${this.mode}`);
        }
        if (this.llmClient.formatToolsForProvider) {
            ...
            const providerFormats = (0, openai_tool_adapter_1.adaptToolDefinitionsToOpenAI)(definitionsToFormat);
     ...
     * @throws {InvalidStateError} if the manager or necessary components are not initialized.
     */
    async getPrimaryLLMSystemPrompt(customBusinessContext) {
        this.assertInitialized();
        const businessContext = customBusinessContext || this.options.businessContextText || '';
        if (this.mode === 'genericOpenApi') {
            if (!this.genericToolProvider || !(this.genericToolProvider instanceof connector_1.OpenAPIConnector)) {
                throw new errors_1.InvalidStateError("Generic tool provider (must be OpenAPIConnector) not available or of wrong type in 'genericOpenApi' mode for prompt generation.");
            }
            const connector = this.genericToolProvider;
            ...
            return (0, prompt_builder_1.generateGenericHttpToolSystemPrompt)(operations, specInfo, baseUrl, businessContext);
        }
        else if (this.mode === 'toolsetsRouter') {
            if (!this.toolsetOrchestrator) {
                throw new errors_1.InvalidStateError('ToolsetOrchestrator not available for prompt generation in toolsetsRouter mode.');
            }
            ...
            if (toolsets.length > 0 && toolsets[0].metadata?.apiTitle) {
                apiInfo = {
                    title: toolsets[0].metadata.apiTitle,
                    version: toolsets[0].metadata.apiVersion || '1.0',
                };
            }
            return (0, prompt_builder_1.generateRouterSystemPrompt)(toolsets, apiInfo, businessContext);
        }
        else if (this.mode === 'hierarchicalPlanner') {
            // The planning agent usually has a very specific, detailed prompt.
            // This prompt might also incorporate businessContext.
            let plannerPrompt = planning_agent_1.DEFAULT_PLANNER_SYSTEM_PROMPT; // Imported from '../agents/planning-agent'
            if (businessContext) {
            ...
            throw new errors_1.InvalidStateError(`Unsupported mode for prompt generation: ${this.mode}`);
        }
    }
    /**
     * Helper to create an instance of the DelegateToSpecialistTool, injecting necessary dependencies.
     */
    createDelegateToSpecialistTool() {
        if (!this.toolsetOrchestrator) {
            throw new errors_1.InvalidStateError("ToolsetOrchestrator is required to create DelegateToSpecialistTool but it's not initialized.");
        }
        const deps = {
            ...
            workerAgentImplementation: agents_1.BaseAgent, // Default worker, can be made configurable
            getDefaultRunConfig: () => ({ ...this.defaultAgentRunConfig }), // Pass a function to get a copy of defaults
        };
        return new delegate_to_specialist_tool_1.DelegateToSpecialistTool(deps);
    }
    /**
     ...
     */
    async getRouterToolDefinition(orchestrator) {
        ...
        return {
            name: prompt_builder_1.ROUTER_TOOL_NAME,
            description: 'Delegates a task to a specialist Toolset that can handle a specific category of operations. ' +
            ...
            parameters: [
                ...
                },
                {
                    name: 'toolName',
                    type: 'string',
                    description: 'The name of the tool within the selected Toolset to execute. Ensure this tool is listed for the chosen Toolset.',
                    required: true,
                    schema: { type: 'string' },
                },
                {
                    name: 'toolParameters',
                    type: 'object',
                    description: "The parameters object required by the chosen 'toolName'. Structure must match the specialist tool's parameters.",
                    required: true,
     ...
     */
    createRouterToolProvider(orchestrator) {
        ...
        const routerTool = {
            ...
            execute: async (input) => {
                ...
                try {
                    console.info(`[RouterTool] Executing tool "${toolName}" from toolset "${toolSetId}" with params:`, JSON.stringify(toolParameters).substring(0, 200) +
                        (JSON.stringify(toolParameters).length > 200 ? '...' : ''));
                    return await tool.execute(toolParameters);
                }
                catch (err) {
                    const errorMsg = err instanceof Error ? err.message : String(err);
                    console.error(`[RouterTool] Error executing tool "${toolName}" from toolset "${toolSetId}": ${errorMsg}`, err);
        ...
        return {
            ...
            getTool: async (name) => (name === prompt_builder_1.ROUTER_TOOL_NAME ? routerTool : undefined),
        };
    }
    /**
     ...
     */
    async *runAgentInteraction(threadId, initialTurnMessages, agentRunConfigOverride = {}, existingRunId) {
        this.assertInitialized();
        ...
        if (!effectiveRunConfig.model) {
            throw new errors_1.ConfigurationError("Agent run configuration must include a 'model'.");
        }
        ...
        if (this.mode === 'hierarchicalPlanner' && !effectiveRunConfig.systemPrompt) {
            effectiveRunConfig.systemPrompt = planning_agent_1.DEFAULT_PLANNER_SYSTEM_PROMPT;
        }
        // In runAgentInteraction & continueAgentRunWithToolOutputs
        // ...
        let agentToolProvider;
        // AgentToRunClass will be determined based on mode and this.agentImplementation
        let AgentToRunClass = this.agentImplementation;
        if (this.mode === 'genericOpenApi') {
            if (!this.genericToolProvider) {
                throw new errors_1.InvalidStateError('Generic tool provider not initialized for agent run.');
            }
            ...
            if (!this.toolsetOrchestrator) {
                throw new errors_1.InvalidStateError('Toolset orchestrator not initialized for agent run.');
            }
            ...
            if (!this.toolsetOrchestrator || !this.aggregatedMasterToolProvider) {
                throw new errors_1.InvalidStateError('ToolsetOrchestrator or AggregatedMasterToolProvider not initialized for hierarchical planner.');
            }
            ...
            const effectivelyPlanningAgent = (this.agentImplementation.name === agents_1.BaseAgent.name && !this.options.agentImplementation) || // It's the default BaseAgent
                (this.agentImplementation.name === planning_agent_1.PlanningAgent.name); // It's explicitly PlanningAgent
            if (effectivelyPlanningAgent) {
                AgentToRunClass = planning_agent_1.PlanningAgent; // Override to PlanningAgent
                ...
                agentToolProvider = {
                    ...
                    getTool: async (name) => ((await delegateTool.getDefinition()).name === name ? delegateTool : undefined),
                ...
                if (effectiveRunConfig.systemPrompt === planning_agent_1.DEFAULT_PLANNER_SYSTEM_PROMPT) {
            ...
            throw new errors_1.InvalidStateError(`Agent run not implemented for mode: ${this.mode}`);
        }
        ...
        if (existingRunId) {
            ...
            if (!existingRecord) {
                throw new errors_1.ApplicationError(`Cannot continue run: Agent run with ID "${existingRunId}" not found.`);
            }
            // Allow continuation from 'requires_action', or re-starting 'queued'/'in_progress' for resilience
            const continuableStates = ['requires_action', 'queued', 'in_progress'];
            if (!continuableStates.includes(existingRecord.status)) {
                throw new errors_1.InvalidStateError(`Cannot continue run: Agent run "${existingRunId}" is not in a continuable state (current status: ${existingRecord.status}).`);
            }
        ...
        const responseProcessor = new response_processor_1.LLMResponseProcessor(effectiveRunConfig.responseProcessorConfig);
        const toolExecutor = new tool_executor_1.ToolExecutor(agentToolProvider, effectiveRunConfig.toolExecutorConfig);
        const contextManager = new context_manager_1.ContextManager(this.messageStorage, this.llmClient, effectiveRunConfig.contextManagerConfig // Pass ContextManager specific config
        );
        const agentContext = {
        ...
        try {
            ...
            const appError = error instanceof errors_1.ApplicationError
                ? error
                : new errors_1.ApplicationError(error.message || 'Unhandled agent execution error.');
            ...
            if (finalRunState && !['completed', 'failed', 'cancelled', 'requires_action'].includes(finalRunState.status)) {
                console.warn(`[ApiInteractionManager: ${runId}] Agent run ended without an explicit terminal status event. Marking as failed.`);
                await this.agentRunStorage.updateRun(runId, {
                    status: 'failed',
                    completedAt: new Date(),
                    lastError: {
     ...
     */
    async *continueAgentRunWithToolOutputs(runId, threadId, // Included for context, though runId should be sufficient to fetch run details
    toolOutputs, agentRunConfigOverride = {}) {
        this.assertInitialized();
        const existingRun = await this.agentRunStorage.getRun(runId);
        if (!existingRun) {
            throw new errors_1.ApplicationError(`Cannot continue: Agent run "${runId}" not found.`);
        }
        if (existingRun.status !== 'requires_action') {
            throw new errors_1.InvalidStateError(`Cannot continue: Agent run "${runId}" is not in 'requires_action' state (current status: ${existingRun.status}).`);
        }
        if (existingRun.threadId !== threadId) {
            throw new errors_1.ConfigurationError(`Thread ID mismatch for run "${runId}". Expected "${existingRun.threadId}", got "${threadId}".`);
        }
        // Use the config from the existing run, merged with any overrides for this continuation.
        const effectiveRunConfig = {
            ...existingRun.config, // Assume stored config is valid AgentRunConfig
            ...agentRunConfigOverride,
            // Ensure model is preserved or correctly overridden
            model: agentRunConfigOverride.model || existingRun.config.model,
        };
        if (!effectiveRunConfig.model) {
            throw new errors_1.ConfigurationError("Agent run configuration (original or override) must include a 'model'.");
        }
        let agentToolProvider;
        // AgentToRunClass will be determined based on mode and this.agentImplementation
        let AgentToRunClass = this.agentImplementation;
        if (this.mode === 'genericOpenApi') {
            if (!this.genericToolProvider) {
                throw new errors_1.InvalidStateError('Generic tool provider not initialized for agent continuation.');
            }
            agentToolProvider = this.genericToolProvider;
            // AgentToRunClass remains this.agentImplementation (e.g., BaseAgent)
        }
            ...
            if (!this.toolsetOrchestrator) {
                throw new errors_1.InvalidStateError('Toolset orchestrator not initialized for agent continuation.');
            }
            agentToolProvider = this.createRouterToolProvider(this.toolsetOrchestrator);
            // AgentToRunClass remains this.agentImplementation (e.g., BaseAgent), sees only RouterTool
        }
            ...
            if (!this.toolsetOrchestrator || !this.aggregatedMasterToolProvider) {
                throw new errors_1.InvalidStateError('ToolsetOrchestrator or AggregatedMasterToolProvider not initialized for hierarchical planner continuation.');
            }
            const effectivelyPlanningAgent = (this.agentImplementation.name === agents_1.BaseAgent.name && !this.options.agentImplementation) ||
                (this.agentImplementation.name === planning_agent_1.PlanningAgent.name);
            if (effectivelyPlanningAgent) {
                AgentToRunClass = planning_agent_1.PlanningAgent;
                if (!effectiveRunConfig.systemPrompt || effectiveRunConfig.systemPrompt === this.defaultAgentRunConfig.systemPrompt) {
                    effectiveRunConfig.systemPrompt = planning_agent_1.DEFAULT_PLANNER_SYSTEM_PROMPT;
                }
                const delegateTool = this.createDelegateToSpecialistTool();
            ...
            throw new errors_1.InvalidStateError(`Continuation not defined for mode ${this.mode}`);
        }
        ...
        const responseProcessor = new response_processor_1.LLMResponseProcessor(effectiveRunConfig.responseProcessorConfig);
        const toolExecutor = new tool_executor_1.ToolExecutor(agentToolProvider, effectiveRunConfig.toolExecutorConfig);
        const contextManager = new context_manager_1.ContextManager(this.messageStorage, this.llmClient, effectiveRunConfig.contextManagerConfig);
        const agentContext = {
            runId,
            threadId,
            llmClient: this.llmClient,
        ...
        if (!agent.submitToolOutputs) {
            throw new errors_1.InvalidStateError(`The configured agent implementation ("${AgentToRunClass.name}") does not support 'submitToolOutputs'.`);
        }
        ...
        try {
            ...
            const appError = error instanceof errors_1.ApplicationError
                ? error
                : new errors_1.ApplicationError(error.message || 'Unhandled agent continuation error.');
            yield {
                type: 'thread.run.failed',
                timestamp: new Date(),
                runId,
     ...
     */
    async updateAuthentication(newAuth) {
        this.assertInitialized();
        if (this.mode === 'genericOpenApi') {
            if (typeof newAuth === 'function') {
                ...
                console.warn("[ApiInteractionManager] updateAuthentication for 'genericOpenApi' mode was called with a function. " +
                ...
                const currentOpts = this.genericToolProvider instanceof connector_1.OpenAPIConnector
                    ? this.genericToolProvider.options // Unsafe cast to access original options
                    : undefined;
                const authToSet = newAuth('genericProvider', currentOpts || {});
                if (authToSet && this.genericToolProvider instanceof connector_1.OpenAPIConnector) {
                    ...
                    console.warn('[ApiInteractionManager] Auth callback returned undefined for genericOpenApi provider; no update performed.');
                }
                else {
                    console.warn('[ApiInteractionManager] Could not apply function-based auth update to genericOpenApi provider (not an OpenAPIConnector or callback failed).');
                }
            }
            else if (this.genericToolProvider instanceof connector_1.OpenAPIConnector) {
                ...
                console.warn('[ApiInteractionManager] genericOpenApi provider is not an OpenAPIConnector instance; cannot set authentication directly.');
            }
        }
        else if (this.mode === 'toolsetsRouter') {
            if (typeof newAuth !== 'function') {
                throw new errors_1.ConfigurationError("For 'toolsetsRouter' mode, updateAuthentication expects a callback function: " +
                    '(sourceId: string, currentOptions: OpenAPIConnectorOptions) => ConnectorAuthentication | undefined.');
            }
            if (!this.toolsetOrchestrator) {
                throw new errors_1.InvalidStateError('ToolsetOrchestrator not initialized for authentication update.');
            }
            await this.toolsetOrchestrator.updateAuthenticationForAllOpenAPIProviders(newAuth);
            console.info('[ApiInteractionManager] Authentication update process initiated for toolset providers via ToolsetOrchestrator.');
        }
        else {
            console.warn('[ApiInteractionManager] No compatible provider found to update authentication for the current mode.');
        }
    }
    /**
    ...
    async getToolset(toolsetId) {
        ...
        if (this.mode !== 'toolsetsRouter' || !this.toolsetOrchestrator) {
            console.warn("[ApiInteractionManager] getToolset is only available in 'toolsetsRouter' mode and when orchestrator is initialized.");
            return undefined;
        }
     ...
     */
    async getAllToolsets() {
        this.assertInitialized();
        if (this.mode !== 'toolsetsRouter' || !this.toolsetOrchestrator) {
            console.warn("[ApiInteractionManager] getAllToolsets is only available in 'toolsetsRouter' mode and when orchestrator is initialized.");
            return [];
        }
     ...
     */
    async getAllGenericTools() {
        this.assertInitialized();
        if (this.mode !== 'genericOpenApi' || !this.genericToolProvider) {
            console.warn("[ApiInteractionManager] getAllGenericTools is only available in 'genericOpenApi' mode and when provider is initialized.");
            return [];
        }
        return this.genericToolProvider.getTools();
    }
}
exports.ApiInteractionManager = ApiInteractionManager;
//# sourceMappingURL=api-interaction-manager.js.map