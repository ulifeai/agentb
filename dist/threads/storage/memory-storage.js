...
class MemoryStorage {
    constructor() {
        ...
        this.agentRuns = new Map(); // Add storage for agent runs
    ...
    async updateThread(threadId, updates) {
        if (!this.threads.has(threadId)) {
            throw new errors_1.StorageError(`Thread with ID "${threadId}" not found for update.`);
        }
    ...
    async listThreads(filterOptions, paginationOptions) {
    ...
    async addMessage(messageData) {
        if (!messageData.threadId) {
            throw new errors_1.ValidationError('threadId is required to add a message.');
        }
        if (!this.threads.has(messageData.threadId)) {
            throw new errors_1.StorageError(`Thread with ID "${messageData.threadId}" not found. Cannot add message.`);
        }
    ...
    async getMessages(threadId, options) {
        if (!this.messages.has(threadId)) {
            ...
            if (!this.threads.has(threadId)) {
                throw new errors_1.StorageError(`Thread with ID "${threadId}" not found when trying to get messages.`);
            }
    ...
    async updateMessage(messageId, updates) {
        ...
        throw new errors_1.StorageError(`Message with ID "${messageId}" not found for update.`);
    }
    ...
    async createRun(runData) {
        const runId = runData.id || (0, uuid_1.v4)();
        const now = new Date();
        const newRun = {
            id: runId,
    ...
    async updateRun(runId, updates) {
        ...
        if (!existingRun) {
            throw new errors_1.StorageError(`Agent run with ID "${runId}" not found for update.`);
        }
        ...
        if (updates.status && updates.status !== existingRun.status) {
            if (updates.status === 'in_progress' && !updatedRun.startedAt)
